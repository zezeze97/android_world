...
| Task | 目标行为 | 初始化数据 | 验证方式 |
| --- | --- | --- | --- |
| `MarkorCreateNoteInFolder` | 在指定 Markor 子文件夹中创建 note | 创建目标目录，并生成干扰 note | 检查目标目录下指定文件存在且内容等于目标文本 |
| `MarkorCreateChecklistNote` | 创建 unchecked checklist note | 无需预置目标文件 | 检查新文件内容为 `- [ ] item` 格式的 checklist |
| `MarkorAppendToNote` | 给已有 note 末尾追加一行文本 | 预置目标 note 和干扰 note | 检查文件内容为原内容、换行、追加文本 |
| `MarkorCopyNote` | 将已有 note 内容复制到新 note，原文件保留 | 预置 source note 和干扰 note | 检查 source 与 new note 均存在且内容一致 |
| `MarkorDeleteNoteInFolder` | 删除指定子文件夹中的 note | 预置待删除文件和干扰文件 | 使用文件删除验证逻辑确认目标文件不存在 |
| `MarkorPrependDateToNote` | 在已有 note 第一行插入日期 | 预置目标 note 和干扰 note | 检查文件内容为日期、换行、原内容 |
| `MarkorReplaceTextInNote` | 替换已有 note 中的指定文本 | 预置包含旧文本的目标 note 和干扰 note | 检查文件内容精确等于替换后的文本 |
| `MarkorRenameNote` | 重命名 note 且内容不变 | 预置 original note 和干扰 note | 检查旧文件不存在、新文件存在且内容不变 |
...
| Task | 目标行为 | 初始化数据 | 验证方式 |
| --- | --- | --- | --- |
| `SimpleCalendarAddEventWithLocation` | 创建带 location 的单个日历事件 | 插入干扰事件 | 检查新增事件的时间、标题、描述、location 和时长 |
| `SimpleCalendarAddLongEvent` | 创建 90/120/180 分钟的长事件 | 插入干扰事件 | 检查新增事件的 start/end 时间和内容字段 |
| `SimpleCalendarAddEventNextWeek` | 按“next weekday”创建下周事件 | 插入干扰事件 | 检查新增事件落在下周指定 weekday |
| `SimpleCalendarAddTwoEventsSameDay` | 在同一天创建两个不同时间的事件 | 插入避开目标日的干扰事件 | 检查两个目标事件都被新增，且日期相同、时间不同 |
| `SimpleCalendarAddTwoEventsDifferentDays` | 在两个不同日期创建两个事件 | 插入避开目标日期的干扰事件 | 检查两个目标事件都被新增，且日期不同 |
| `SimpleCalendarDeleteEventByTitle` | 只根据标题删除一个事件 | 预置目标事件和不同标题的干扰事件 | 检查目标事件被删除，其他事件保留 |
| `SimpleCalendarDeleteEventByTitleAndDate` | 根据标题和日期删除一个事件 | 预置目标事件和干扰事件 | 检查匹配标题与日期的事件被删除 |
| `SimpleCalendarDeleteTomorrowEvents` | 删除明天的所有事件 | 预置明天的目标事件和非明天干扰事件 | 检查明天目标事件被删除，其他日期事件保留 |
