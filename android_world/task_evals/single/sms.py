...
class SimpleSmsForwardDoorCode(sms_validators.SimpleSMSSendSms):
  """Task for forwarding a received door code to a saved contact."""
  ...
  def initialize_task(self, env: interface.AsyncEnv) -> None:
    ...
    adb_utils.text_emulator(
        env.controller,
        self.params["sender_number"],
        f"Front desk note: today's door code is {self.params['code']}.",
    )
    time.sleep(1)
    adb_utils.enable_headsup_notifications(env.controller)
...
class SimpleSmsForwardAppointmentTime(sms_validators.SimpleSMSSendSms):
  """Task for forwarding only an appointment time to a saved contact."""
  ...
  def initialize_task(self, env: interface.AsyncEnv) -> None:
    ...
    adb_utils.text_emulator(
        ...
        self.params["sender_number"],
        (
            "Appointment reminder: please arrive at"
            f" {self.params['code']} and bring ID."
        ),
    )
    time.sleep(1)
    adb_utils.enable_headsup_notifications(env.controller)
  ...
  @classmethod
  def generate_random_params(cls) -> dict[str, str | int]:
    appointment_time = random.choice(["8:30 AM", "10:15 AM", "2:45 PM"])
    return {
        "recipient_name": user_data_generation.generate_random_name(),
        "number": user_data_generation.generate_random_number(),
        "sender_name": "Clinic Desk",
        "sender_number": user_data_generation.generate_random_number(),
...
class SimpleSmsSendNoteSnippetToSavedContact(
    sms_validators.SimpleSMSSendSms
):
  """Task for sending a note snippet from clipboard to a saved contact."""
  ...
  def initialize_task(self, env: interface.AsyncEnv) -> None:
    ...
    adb_utils.set_clipboard_contents(
        f"Snippet to send: {self.params['message']}", env.controller
    )
