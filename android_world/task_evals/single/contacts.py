...
class ContactsAddContractorFromClipboard(contacts_validators.AddContact):
  """Task for adding a contractor contact from clipboard text."""
  ...
  def initialize_task(self, env: interface.AsyncEnv) -> None:
    ...
    adb_utils.set_clipboard_contents(
        (
            f"Contractor contact - name: {self.params['name']}; phone:"
            f" {self.params['number']}"
        ),
        env.controller,
    )
...
class ContactsAddClinicCallbackFromSms(contacts_validators.AddContact):
  """Task for adding a clinic callback contact from an incoming SMS."""

  app_names = ("contacts", "simple sms messenger")
  complexity = 1.8
  schema = ContactsAddFromIncomingSms.schema
  ...
  def initialize_task(self, env: interface.AsyncEnv) -> None:
    ...
    adb_utils.text_emulator(
        ...
        self.params["sender_number"],
        (
            f"Clinic callback contact is {self.params['name']} at"
            f" {self.params['number']}. Save it before the appointment."
        ),
    )
    adb_utils.enable_headsup_notifications(env.controller)
...
class ContactsAddVendorFromMarkorNote(ContactsAddFromMarkorNote):
  """Task for adding the vendor contact from a Markor directory note."""
  ...
  def generate_random_params(cls) -> dict[str, str | int]:
    return {
        "name": user_data_generation.generate_random_name(),
        "number": user_data_generation.generate_random_number(),
        "role": random.choice(["catering vendor", "print shop", "repair desk"]),
    }
